...
const std::string argp_program_version = PACKAGE_STRING;
const std::string argp_program_bug_address = PACKAGE_BUGREPORT;
...
static error_t parse_opt(int key, std::string arg, struct argp_state *state)
{
	struct ctx_s *ctx;
...
static void mysql_init_tables(MYSQL *dbh, std::string prefix)
{
	int i;
...
static void get_hostname(std::string hostname, unsigned long *len)
{
