...
int parse_arp(struct pkt *p)
{
	...
	if (p->len < sizeof(struct ether_arp)) {
		log_msg(LOG_WARNING, "%s: Error parsing ARP packet. Packet is too small (%d of %d bytes)",
			p->ifc->name, p->len, sizeof(struct ether_arp));
		return -2;
	}
...
int parse_nd(struct pkt *p)
{
	...
	if (p->len < sizeof(struct nd_neighbor_solicit)) {
		log_msg(LOG_WARNING, "%s: Error parsing ICMPv6 ND packet. Packet is too small (%d of %d bytes)",
			p->ifc->name, p->len, sizeof(struct nd_neighbor_solicit));
		return -2;
	}
	...
	while (1) {
		...
		if (opt->nd_opt_len == 0) {
			log_msg(LOG_WARNING, "%s: Error parsing ICMPv6 ND options. Option length is 0.",
				p->ifc->name);
			return -2;
		}

		if (p->len < opt->nd_opt_len * 8) {
			log_msg(LOG_WARNING, "%s: Error parsing ICMPv6 ND options. Option header is too small (%d of %d bytes)",
				p->ifc->name, p->len, opt->nd_opt_len * 8);
			return -2;
		}
...
int parse_ipv6(struct pkt *p)
{
	...
	if (p->len < sizeof(struct ip6_hdr)) {
		log_msg(LOG_WARNING, "%s: Error parsing IPv6 packet. Packet is too small (%d of %d bytes)",
			p->ifc->name, p->len, sizeof(struct ip6_hdr));
		return -2;
	}
	...
	if (p->len < sizeof(struct icmp6_hdr)) {
		log_msg(LOG_WARNING, "%s: Error parsing ICMPv6 packet. Header is too small (%d of %d bytes)",
			p->ifc->name, p->len, sizeof(struct icmp6_hdr));
		return -2;
	}
...
int parse_packet(struct pkt *p)
{
	...
	if (p->len < sizeof(struct ether_header)) {
		log_msg(LOG_WARNING, "%s: Error parsing Ethernet packet. Packet is too small (%d of %d bytes)",
			p->ifc->name, p->len, sizeof(struct ether_header));
		return -2;
	}
